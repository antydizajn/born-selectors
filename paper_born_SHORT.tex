...
\textbf{Short version} (Gram scaffolding omitted; see
\texttt{paper\_born.md} full).\\
\textbf{Author:} Paulina Janowska\\
\textbf{Date:} 2026-07-24\\
\textbf{Version:} 4.7-short
...
\item
  ...
  real-bilinear forms on \(\mathrm{Herm}_d\) (Lemma 2);
\item
  \textbf{five selector theorems (Theorems 1--5)} selecting Born inside
  that class;
\item
  Proposition 3: local equivalence of Theorems 1 and 3;
\item
  honest positioning vs Galley--Masanes {[}9{]}, Hossenfelder {[}22{]},
  and Nakahira {[}8{]}.
\end{itemize}

This paper is not a derivation of quantum theory, nor a classification
of all Born alternatives (see {[}9{]}).

\subsubsection{1.3 Related prior work}\label{related-prior-work}

See the full manuscript \texttt{paper\_born.md} for the complete
related-work discussion. In brief: Galley--Masanes {[}9{]} classify
general alternatives keeping pure rays and unitaries; we study only the
bilinear mixed-state slice. Hossenfelder {[}22{]} uses
dimension-independence on the complex sphere. Nakahira {[}8{]} derives
full complex QM from two OPT postulates. Gleason {[}14{]} and Busch
{[}15{]} use additivity.

\subsubsection{1.4 Outline}\label{outline}

Section 2: notation. Section 5: Lemma 2. Section 6: selectors. Section
7: limits. Section 8: Nakahira. (Gram scaffolding of the full manuscript
is omitted here.)
